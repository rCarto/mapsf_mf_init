...
# The `mf_init()` function

The best way to export maps with `mapsf` is with its dedicated function `mf_init()`.   

As its name suggests, `mf_init()` is used to initiate a map in an R session where maps are displayed in the plot window. It can also be used to export maps in PNG or SVG formats. 


## Initiate a map

`mf_init()` plots `x`, an `sf` object, as an invisible layer. Other geographic layers can be overlayed upon this initial layer. 
...
One can also use `mf_init()` to set a theme. 
...
The `expandBB` argument can be used to extend the map space on one or several sides of the figure. 
